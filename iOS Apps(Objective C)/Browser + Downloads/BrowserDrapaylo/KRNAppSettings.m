...
static NSString* plistPath; // путь к plist

NSString* const KRNNotificationsPrefix = @"KRNSET";
...
+(id)sharedSettings
{
    ...
    static KRNAppSettings *sharedSettings = nil;
    ...
    dispatch_once(&onceToken, ^{
        ...
        NSFileManager *fileManager = [NSFileManager defaultManager];
         ...
         NSString *directoryForSettings = NSSearchPathForDirectoriesInDomains(NSLibraryDirectory, NSUserDomainMask, YES).firstObject;
      
        
        NSString *settingsPath = [NSString stringWithFormat:@"%@/%@", directoryForSettings, settingsFileName]; // получаем полный путь
        
        // если в LibraryDirectory отсутствует файл настроек, то нужно его создать с дефолтными настройками
...
-(void)loadSettings
{
    ...
    [KRNAppSettings runAllPropertiesLoop:^(objc_property_t property) {
...
-(void)postNotificationWithSettingsKey:(NSString*)settingsKey andValue:(id) value // запостить нотификацию об изменении значения настройки через Notification Center
{
    ...
    NSString* notificationName = [NSString stringWithFormat:@"%@%@", KRNNotificationsPrefix, settingsKey];
    
    [notifCenter postNotificationName:notificationName object:self userInfo:@{settingsKey:value}];
...
{
    self.maxDownloadTasks = 3; // максимум параллельных загрузок по умолчанию
//    self.catchJPG = self.catchJPEG = self.catchPNG = self.catchPDF = self.catchTXT = YES; // ловим все файлы по умолчанию
    
    self.catchFileTypes = KRNFormatJPG | KRNFormatJPEG | KRNFormatPNG | KRNFormatTXT | KRNFormatPDF; // по умолчанию читаем все форматы
...
-(NSArray<NSString*>*) getSupportedFileTypesInArray // вернуть поддерживаемый тип файлов как массив
{
    ...
    if (self.catchFileTypes & KRNFormatJPG)
        [array addObject:@"jpg"];
    if (self.catchFileTypes & KRNFormatJPEG)
        [array addObject:@"jpeg"];
    if (self.catchFileTypes & KRNFormatPNG)
        [array addObject:@"png"];
    if (self.catchFileTypes & KRNFormatTXT)
        [array addObject:@"txt"];
    if (self.catchFileTypes & KRNFormatPDF)
        [array addObject:@"pdf"];
